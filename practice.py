...
def find_unique_common_items(items1, items2):
    ...
    **IMPORTANT**: you may not use `'if ___ in ___``
    ...
    unique_common_items = set(items1).intersection(set(items2))
    ...
    for num_a in numbers:
        for num_b in numbers:
            if num_a + num_b == 0:
                ...
                zero_pairs.add(min(num_a, num_b))

    for num in zero_pairs:
        sum_pairs.append([num, -num])         
...
def top_chars(phrase):
    ...
    max_value = max(letters_dictionary.values())
